...
@Injectable()
export class ClientCalculatedHatilloService {
  ...
  async getProformasdataHatillo(payload: any) {
      ...
      if (!clientesConNombreCompletos || !Array.isArray(clientesConNombreCompletos) || clientesConNombreCompletos.length === 0) {
        ...
        for (const cliente of clientesConNombreCompletos) {
          ...
          const tasasPorMonedaYFecha = tasasDecambioPa.reduce((acc, tasa) => {
            const { CURNCYID, EXCHDATE, XCHGRATE, EXGTBLID } = tasa;
            ...
            acc[moneda][fecha] = {tasa: XCHGRATE, tipo: EXGTBLID.trim()};
            return acc;
          }, {});
          //console.log(tasasPorMonedaYFecha)
          const obtenerTasasPorFechas = (moneda, fechas) => {
            return fechas.map(fecha => {
              const fechaISO = fecha.toISOString().split('T')[0];
              const tasa = tasasPorMonedaYFecha[moneda]?.[fechaISO];
              return tasa ? { fecha: fechaISO, tasa: tasa.tasa, tipo: tasa.tipo } : null;
            }).filter(Boolean);
          };
          ...
          for (let i = 0; i < fechasEmisionOriginal.length; i++) {
            ...
            //console.log(fechasEmisionOriginal)
            let EUR;
            let USD;
            let PTR;
            let tasabasptr;
            let tasabasearmon;
            //console.log(tasasUSD)
            ...
            for (const tasa of tasasUSD) {
              if (new Date(tasa.fecha).getTime === fechaDate.getTime && tasa.tipo==='VEB/USD') {
                  USD = tasa.tasa;
                  break;
              }
            }
            for (const tasa of tasasPTR) {
              if (new Date(tasa.fecha).getTime === fechaDate.getTime && tasa.tipo==='VEB/PTR') {
                  PTR = tasa.tasa;
                  break;
              }
            ...
            //console.log(fechaDate.getTime() < datearmonizacionn,tasabasearmon, fechaDate)
